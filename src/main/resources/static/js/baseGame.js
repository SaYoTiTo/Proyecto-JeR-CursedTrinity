...
export default class BaseGame extends Phaser.Scene{
    ...
    create() {
        ...
        */
		...
		this.stateMachine.disabled = false;
		...
		this.stateMachine2.disabled = false;
	...
	disableChar(ch){
		if(ch === 0){
			this.stateMachine.disabled = true;
		}else{
			this.stateMachine2.disabled = true;
		}
	}


	getHeroAPos(){
	var pos = [this.heroA.x, this.heroA.y];
	return pos;
	}

	getHeroCPos(){
	var pos = [this.heroC.x, this.heroC.y];
	return pos;
	}
	...
	disableHeroC(ans){
		console.log("Ahora heroC estara a " + ans);
		this.book.disableBody(!ans, !ans);
		this.heroC.disableBody(!ans, !ans);
		this.heroC.setVisible = ans;
	}
	
	disableHeroA(ans){
		console.log("Ahora heroA estara a " + ans);
		this.bow.disableBody(!ans, !ans);
		this.heroA.disableBody(!ans, !ans);
		this.heroA.setVisible = ans;
	}
    ...
    update() {
        ...
        if(document.readyState === 'complete'){
            ...
            if(this.heroA.lifes > 0 && this.stateMachine.disabled === false){
            ...
            */
            if(this.heroC.lifes > 0 && this.stateMachine2.disabled === false){ 
...
var s = function(pointer) {
    if(pointer.leftButtonDown() && this.heroA.lifes > 0 && sizeFlechas > 0 && boolHeroA != false){
...
var s2 = function(pointer) {
    if(pointer.rightButtonDown() && this.heroC.lifes > 0 && boolHeroC != false){
