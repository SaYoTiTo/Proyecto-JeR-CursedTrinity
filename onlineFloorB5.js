import {BaseGame} from './baseGame.js';
import {game} from './game.js';

WSconnection = new WebSocket('');

var posX;
var posY;
var constanciaEscenas;


export default class OnlineFloorB5 extends BaseGame {
    constructor(){
        super('OnlineFloorB5');
    }
    
    init(data){
        this.lifeA = data.lifeA;
        this.lifeC = data.lifeC;
        this.arrows = data.arrows;
        this.exitX = data.exitX;
        this.exitY = data.exitY;
    }

    create(){
        this.cameras.main.fadeIn(250,0,0,0);
        
        super.setCurrentScene('OnlineFloorB5');
        console.log(this.registry.get('oB5'));
        super.restartPos();
        super.resetLevers();
        super.resetRocks();
        if(this.registry.get('oB5') === false){
            super.setHeroAx(180, 60);
            super.setHeroCx(240, 50);
            super.setReward(false, false);
            super.setOpenableDoors(0, 1, 2, 3);
            super.setNumEnemies(3,0,0);
            super.setBossDeath(false);
            super.create();
            super.closeDoor(0);
            
            super.setHeroALifes(this.lifeA);
            super.setHeroCLifes(this.lifeC);
            
        }else{
            if(this.exitX > 0.5){
                super.setHeroAx(75, 140);
                super.setHeroCx(75, 180);
            }else if(this.exitX < 0.5){
                super.setHeroAx(345, 140);
                super.setHeroCx(345, 180);
            }
            if(this.exitY > 0.5){
                super.setHeroAx(180, 60);
                super.setHeroCx(240, 50);
            }else if(this.exitY < 0.5){
                super.setHeroAx(180, 240);
                super.setHeroCx(240, 230);
            }
            super.setReward(false, false);
            super.setNumEnemies(0,0,0);
            super.setBossDeath(false);
            super.create();
			super.startOpenDoor(0);
			super.startOpenDoor(1);
			super.startOpenDoor(2);
            super.startOpenDoor(3);
            super.setHeroALifes(this.lifeA);
            super.setHeroCLifes(this.lifeC);
        }

        super.heroA.disableBody(true, true);

    }
    update(){
        super.update();           
            
        posX = super.getPosX();
        posY = super.getPosY();
        
		if(posX > 0.5){
            this.cameras.main.fadeOut(250,0,0,0);
            this.registry.set('oB5', true);
            this.scene.stop('OnlineFloorB5');
            this.scene.start('OnlineFloorB6', 
                             { lifeA: super.getHeroALifes(), lifeC: super.getHeroCLifes(), arrows: super.getArrows(), exitX: 1, exitY: 0.5 });
        }
		
		if(posX < 0.5){
			this.cameras.main.fadeOut(250,0,0,0);
            this.registry.set('oB5', true);
            this.scene.stop('OnlineFloorB5');
            this.scene.start('OnlineFloorB4', 
                             { lifeA: super.getHeroALifes(), lifeC: super.getHeroCLifes(), arrows: super.getArrows(), exitX: 0, exitY: 0.5 });
		}
		
		if(posY > 0.5){
            this.cameras.main.fadeOut(250,0,0,0);
            this.registry.set('oB5', true);
            this.scene.stop('OnlineFloorB5');
            this.scene.start('OnlineFloorC5', 
                             { lifeA: super.getHeroALifes(), lifeC: super.getHeroCLifes(), arrows: super.getArrows(), exitX: 0.5, exitY: 1 });
        }
		
		if(posY < 0.5){
			this.cameras.main.fadeOut(250,0,0,0);
            this.registry.set('oB5', true);
            this.scene.stop('OnlineFloorB5');
            this.scene.start('OnlineFloorA5', 
                             { lifeA: super.getHeroALifes(), lifeC: super.getHeroCLifes(), arrows: super.getArrows(), exitX: 0.5, exitY: 0 });
		}
    }
}

function updatePalancas(obj){
    
    //Palancas de Arrow
    this.registry.set('p1Arrow', obj.p1);
    this.registry.set('p2Arrow', obj.p2);

}

WSconnection.onmessage = function(msg){
    var obj = JSON.parse(msg.data);

    if(obj.typePetition === 1)
        //Updatear palancas
        updatePalancas(obj);
}